...
var nicEditorBgColorButton = nicEditorColorButton.extend({
	colorSelect : function(c) {
		//this.ne.nicCommand('hiliteColor',c);BackColor
...
var nicTableOptions = {
	buttons : {
		'table' : {name : 'Add Table', type : 'nicTableButton'}
	}
...
var nicTableButton = nicEditorAdvancedButton.extend({	
	...
	addPane : function() {
		this.addForm({
			'' : {type : 'title', txt : 'Add Table'},
			'column' : {type : 'text', txt : 'Columns'},
			'row' : {type : 'text', txt : 'Rows'},
		});
	},
	
	submit : function(e) {
		var iRows = this.inputs['row'].value;
		var iCols = this.inputs['column'].value;
		if(iRows == "" || iCols == "") {
			alert("You must enter a Columns and Rows value");
			return false;
		}
				
		this.removePane();
		
		var body= this.ne.selectedInstance.getElm();
        ...
        for (var j = 0; j < iRows; j++) {
            ...
            for (var i = 0; i < iCols; i++) {
				...
				cell.style.width='150px';cell.style.height='20px';
...
nicTableButton.statusCb = function(o) {
	nicTableButton.lastPlugin.update(o);
}
